/**
 * Busca leads REAIS por funil — verifica pipeline_id do lead
 * pra garantir que pertence ao funil correto
 */
...
async function main() {
  console.log("🔍 BUSCANDO LEADS REAIS — VERIFICANDO PIPELINE_ID");
  ...
  for (const team of ["azul", "amarela"] as TeamKey[]) {
    ...
    for (const pipeline of pipelines) {
      ...
      for (const rule of rules) {
        ...
        // Buscar leads nessa etapa — paginar até achar 1 que REALMENTE pertence a este pipeline
        let page = 1;
        let found = false;
        ...
        while (page <= 5 && !found) {
          ...
          for (const lead of leads) {
            // VERIFICACAO CRITICA: o lead PERTENCE a este pipeline?
            if (lead.pipeline_id !== pipeline.id) continue;
            // O lead esta NESTE status?
            ...
            if (rule.requiresNoNotes) console.log(`   Notas: 0 (sem nota)`);
